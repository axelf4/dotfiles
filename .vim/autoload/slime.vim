...
function slime#Operator(...) abort
	...
	function! s:WindowCb(target) abort closure
		let b:slime_target = a:target
		call s:TmuxSend(a:target, text)
	endfunction
	if exists('b:slime_target')
		call s:WindowCb(b:slime_target)
	else
		call s:PromptWindow(funcref('s:WindowCb'))
	endif
endfunction
...
function s:PromptWindow(Cb) abort
	...
	function! s:DrawCell(node, x, y, w, h) abort closure
		let type = a:node[0]
		if type ==# 'leaf'
			...
			for i in range(nchildren)
				...
				call s:DrawCell(a:node[1][i], cx, cy, cw, ch)
	...
	silent call s:DrawCell(head, 1, 1, w, h)
...
function s:TmuxSend(target, text) abort
	...
	call system(printf('tmux send-keys -t %s %s %s', target, s:tmux_reset_seq, shellescape(a:text)))
endfunction
